...
import { Topic, Question, ClinicalCase } from '../types';
...
export default function LessonModal({ topic, onClose, onComplete }: LessonModalProps) {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'yuanwen' | 'baihua' | 'anli' | 'koujue' | 'jinji'>('all');
  ...
  const renderFormattedMarkdown = (
    text: string, 
    themeColor: 'cinnabar' | 'teal' | 'amber' | 'purple' | 'rose' | 'zinc' = 'zinc'
  ): React.ReactNode => {
    if (!text) return null;
  ...
  const renderCardBlock = (blockText: string, blockIdx: number) => {
    ...
    if (trim.startsWith('### ')) {
      ...
      return (
        <h4 key={blockIdx} className="text-sm sm:text-base font-extrabold text-[#292524] dark:text-[#f5f5f4] flex items-center gap-2 pt-3 pb-2 border-b border-[#e7dfd3] dark:border-[#38322c]">
          <span className="w-2.5 h-2.5 rounded-full bg-[#b91c1c] dark:bg-[#ef4444] shrink-0"></span>
    ...
    if (trim.startsWith('## ')) {
      const headingText = trim.replace('## ', '');
      return (
        <h3 key={blockIdx} className="text-base sm:text-lg font-bold text-[#1c1917] dark:text-[#fafaf9] pt-3 pb-1 border-b-2 border-[#b91c1c] dark:border-[#ef4444]">
    ...
    if (
      ...
      const cleaned = trim
        .replace(/^[-* ]*\*\*(理论来源|经典依据|原文)\*\*[:：]?\s*/i, '')
        .replace(/^[-* ]*(理论来源|经典依据|原文)[:：]?\s*/i, '')
        .replace(/^📖\s*/, '');

      return (
        <div key={blockIdx} className="bg-[#faf2f2] dark:bg-[#2d1515] border-l-4 border-[#b91c1c] dark:border-[#ef4444] rounded-r-2xl rounded-l-xs p-4 text-xs md:text-sm space-y-2 shadow-xs my-3.5 border border-[#f5d0d0]/60 dark:border-[#4a1d1d]/80">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-[#881337] dark:text-[#fca5a5] font-extrabold text-xs tracking-wider">
              <span className="px-2 py-0.5 rounded-md bg-[#b91c1c] text-white text-[11px] font-mono shadow-2xs">📜 原文</span>
              <span>经典出处与依归</span>
            </div>
            <span className="text-[10px] font-mono text-[#a16207] dark:text-[#fde047] bg-[#fef9c3] dark:bg-[#713f12] px-2 py-0.5 rounded-full border border-[#fef08a] dark:border-[#a16207]">
              经方藏经阁
            </span>
          </div>
          <div className="text-[#701a1a] dark:text-[#fecdd3] font-serif leading-relaxed font-semibold italic text-sm sm:text-base bg-white/70 dark:bg-black/30 p-3 rounded-xl border border-[#f87171]/20">
    ...
    // COLOR BLOCK 2: 【白话解读 / 物理内景】 - 黛青 (Dai Cyan / Indigo)
    if (
      trim.includes('白话解读') || 
      trim.includes('物理内景') || 
      trim.includes('理论通解') || 
      ...
      return (
        <div key={blockIdx} className="bg-[#f0f7f7] dark:bg-[#0f282a] border-l-4 border-[#0d5d56] dark:border-[#14b8a6] rounded-r-2xl rounded-l-xs p-4 text-xs md:text-sm space-y-2 shadow-xs my-3.5 border border-[#c2f0ec]/60 dark:border-[#134e4a]/80">
          <div className="flex items-center gap-1.5 text-[#042f2e] dark:text-[#99f6e4] font-extrabold text-xs tracking-wider">
            <span className="px-2 py-0.5 rounded-md bg-[#0d5d56] text-white text-[11px] font-mono shadow-2xs">💡 白话解读</span>
            <span>物理内景与机制通解</span>
          </div>
          <div className="text-[#115e59] dark:text-[#ccfbf1] leading-relaxed font-medium">
    ...
    // COLOR BLOCK 3: 【临床案例】 - 赭石 / 琥珀 (Ocher / Warm Amber)
    if (
      trim.includes('临床案例') || 
      trim.includes('误辨案例') || 
      trim.includes('实战案例') || 
      ...
      const formattedLines = cleaned.split('\n').map((line, lIdx) => {
        if (line.includes('分析:') || line.includes('分析：')) {
          const parts = line.split(/分析[:：]/);
          return (
            <p key={lIdx} className="pt-1 text-[#78350f] dark:text-[#fef3c7] font-medium">
              <strong className="text-white bg-[#b45309] dark:bg-[#d97706] px-1.5 py-0.5 rounded mr-1 text-xs font-bold shadow-2xs">分析：</strong>
              {renderFormattedMarkdown(parts[1] || '', 'amber')}
            </p>
          );
        }
        if (line.includes('思维链:') || line.includes('思维链：')) {
          const parts = line.split(/思维链[:：]/);
          return (
            <div key={lIdx} className="bg-[#fef3c7] dark:bg-[#78350f]/80 p-3 rounded-xl border border-[#fde68a] dark:border-[#b45309] my-2 shadow-2xs">
              <strong className="text-[#92400e] dark:text-[#fef3c7] font-extrabold text-xs block mb-1 flex items-center gap-1">
                <span>🔗 诊疗逻辑思维链：</span>
              </strong>
              <span className="font-mono text-xs text-[#78350f] dark:text-[#fef3c7] font-bold">{parts[1] || ''}</span>
            </div>
          );
        }
        if (line.includes('患者主诉') || line.includes('主诉')) {
          return (
            <p key={lIdx} className="text-[#78350f] dark:text-[#fde68a] font-medium bg-[#fef9c3]/80 dark:bg-[#451a03]/60 p-2.5 rounded-lg border border-[#fef08a]/60 dark:border-[#78350f]/60">
              {renderFormattedMarkdown(line, 'amber')}
            </p>
          );
        }
        if (line.includes('来源:') || line.includes('来源：')) {
          return (
            <p key={lIdx} className="text-[11px] text-[#92400e] dark:text-[#fde68a] italic pt-1">
              {line}
            </p>
          );
        }
        return <p key={lIdx} className="leading-relaxed font-medium">{renderFormattedMarkdown(line, 'amber')}</p>;
      });

      return (
        <div key={blockIdx} className="bg-[#fdf8ee] dark:bg-[#2a1d12] border-l-4 border-[#b45309] dark:border-[#f59e0b] rounded-r-2xl rounded-l-xs p-4 text-xs md:text-sm text-[#78350f] dark:text-[#fef3c7] space-y-2 shadow-xs my-3.5 border border-[#fde68a]/60 dark:border-[#78350f]/80">
          <div className="flex items-center gap-1.5 text-[#78350f] dark:text-[#fde68a] font-extrabold text-xs tracking-wider">
            <span className="px-2 py-0.5 rounded-md bg-[#b45309] text-white text-[11px] font-mono shadow-2xs">📋 临床案例</span>
            <span>实战研案与辨析</span>
          </div>
    ...
    // COLOR BLOCK 4: 【记忆口诀】 - 紫檀 / 绛紫 (Imperial Purple)
    if (
      trim.includes('记忆口诀') || 
      trim.includes('口诀') || 
      trim.includes('歌诀') || 
      ...
      return (
        <div key={blockIdx} className="bg-[#f8f5fa] dark:bg-[#241733] border-l-4 border-[#7e22ce] dark:border-[#c084fc] rounded-r-2xl rounded-l-xs p-4 text-xs md:text-sm text-[#581c87] dark:text-[#f3e8ff] space-y-2 shadow-xs my-3.5 font-mono border border-[#e9d5ff]/60 dark:border-[#581c87]/80">
          <div className="flex items-center gap-1.5 text-[#581c87] dark:text-[#f3e8ff] font-extrabold text-xs tracking-wider">
            <span className="px-2 py-0.5 rounded-md bg-[#7e22ce] text-white text-[11px] font-mono shadow-2xs">🔑 记忆口诀</span>
            <span>速记经方临证歌诀</span>
          </div>
          <div className="leading-relaxed font-bold text-sm bg-white/80 dark:bg-black/30 p-3 rounded-xl border border-[#c084fc]/30 text-center tracking-wide text-[#6b21a8] dark:text-[#e9d5ff]">
            {renderFormattedMarkdown(cleaned, 'purple')}
          </div>
        </div>
      );
    }

    // COLOR BLOCK 5: 【易错点与临床禁忌】 - 朱红 WARNING (Cinnabar Red Alert)
    if (
      trim.includes('易错点') || 
      trim.includes('辨析禁忌') || 
      trim.includes('临床禁忌') || 
      ...
      return (
        <div key={blockIdx} className="bg-[#fff5f5] dark:bg-[#321616] border-l-4 border-[#dc2626] dark:border-[#f87171] rounded-r-2xl rounded-l-xs p-4 text-xs md:text-sm text-[#991b1b] dark:text-[#fca5a5] space-y-2 shadow-xs my-3.5 border border-[#fecdd3]/60 dark:border-[#7f1d1d]/80">
          <div className="flex items-center gap-1.5 text-[#991b1b] dark:text-[#fca5a5] font-extrabold text-xs tracking-wider">
            <span className="px-2 py-0.5 rounded-md bg-[#dc2626] text-white text-[11px] font-mono shadow-2xs">⚠️ 临证误区与禁忌</span>
            <span>警惕盲目误用</span>
          </div>
          <div className="leading-relaxed font-semibold">{renderFormattedMarkdown(cleaned, 'rose')}</div>
        </div>
      );
    }

    // Check if text has numbered items like "1. **口苦 = 肝胆问题？** 错！... 2. **头痛...**"
    if (/^\d+\.\s+/.test(trim)) {
      ...
      return (
        <div key={blockIdx} className="space-y-3 my-3">
          {items.map((it, idx) => {
            ...
            let badgeTag = null;
            let textBody = cleanedItem;

            if (cleanedItem.includes('错！') || cleanedItem.includes('错!')) {
              const parts = cleanedItem.split(/错[！!]/);
              textBody = parts[1] || parts[0];
              badgeTag = (
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-[#ffe4e6] dark:bg-[#881337] text-[#9f1239] dark:text-[#fecdd3] border border-[#fecdd3] dark:border-[#be123c] shrink-0">
                  ❌ 常见误区
                </span>
              );
            }
            ...
            return (
              <div 
                key={idx} 
                className="flex gap-3 items-start text-xs sm:text-sm text-[#1c1917] dark:text-[#f5f5f4] leading-relaxed bg-[#f5f0e6] dark:bg-[#2e2a25] p-3.5 rounded-2xl border border-[#e2d8c7] dark:border-[#443e37] shadow-2xs"
              >
                <span className="w-5 h-5 rounded-full bg-[#b91c1c] text-white font-bold text-[11px] flex items-center justify-center shrink-0 mt-0.5 shadow-2xs font-mono">
                  {idx + 1}
                </span>
                <div className="space-y-1.5 flex-1">
                  {badgeTag && <div className="mb-1">{badgeTag}</div>}
                  <div className="font-medium leading-relaxed">
                    {renderFormattedMarkdown(textBody, 'zinc')}
                  </div>
                </div>
              </div>
            );
            ...
            if (/^[*•-]\s+/.test(cleanLine)) {
              ...
              if (keyValueMatch) {
                ...
                return (
                  <div key={lIdx} className="flex flex-col sm:flex-row sm:items-start gap-2 bg-[#f5f0e6] dark:bg-[#2e2a25] p-3 rounded-2xl border border-[#e2d8c7] dark:border-[#443e37] shadow-2xs">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-extrabold bg-[#b91c1c] text-white shrink-0 shadow-2xs w-fit">
                      【 {keyTerm} 】
                    </span>
              ...
              return (
                <div key={lIdx} className="flex gap-2.5 items-start text-xs sm:text-sm text-[#1c1917] dark:text-[#f5f5f4] leading-relaxed bg-[#f7f3e9]/80 dark:bg-[#27231f]/80 p-3 rounded-xl border border-[#e2d8c7]/80 dark:border-[#38322c]/80">
                  <span className="w-2 h-2 rounded-full bg-[#0d5d56] shrink-0 mt-1.5"></span>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#1c1917]/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 animate-fadeIn">
      <div className="w-full max-w-xl bg-[#f8f4eb] dark:bg-[#1a1715] rounded-3xl border border-[#e2d8c7] dark:border-[#3a332c] shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
              ...
              >
                ...
                <div className={`[backface-visibility:hidden] bg-[#fffcf7] dark:bg-[#24201d] border border-[#ebdcc8] dark:border-[#3a332a] rounded-3xl p-5 sm:p-7 shadow-xl space-y-5 ${isFlipped ? 'pointer-events-none' : ''}`}>
                  ...
                  <div className="flex items-center justify-between border-b border-[#f5f0e6] dark:border-[#2e2a25] pb-3">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold bg-[#faf2f2] dark:bg-[#2d1515] text-[#b91c1c] dark:text-[#fca5a5] border border-[#f5d0d0] dark:border-[#4a1d1d]">
                        <Lightbulb className="w-3.5 h-3.5 text-[#b91c1c]" />
                        经方知识点 {currentStep + 1} / {totalLessons}
                      </span>
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-extrabold bg-[#f5f0e6] hover:bg-[#b91c1c] hover:text-white dark:bg-[#2e2a25] dark:hover:bg-[#ef4444] text-[#0d5d56] dark:text-[#5eead4] transition-all border border-[#e2d8c7] dark:border-[#443e37] shadow-2xs cursor-pointer group"
            ...
            <div className="bg-[#fffcf7] dark:bg-[#24201d] border border-[#ebdcc8] dark:border-[#3a332a] rounded-3xl p-5 sm:p-7 shadow-lg space-y-5 animate-fadeIn">
              ...
              {isAnswerChecked && (
                <div className={`p-4 rounded-2xl border text-xs space-y-1.5 animate-fadeIn ${
                  ...
                  <p className="leading-relaxed pt-1.5 border-t border-current/10 font-mono">
                    <strong>内景解析：</strong>{quizQuestions[currentQuizIdx].explanation}
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="px-5 py-3.5 bg-[#f4efe4] dark:bg-[#231f1c] border-t border-[#e2d8c7] dark:border-[#38322c] flex justify-between items-center">
          <button
            onClick={handlePrevStep}
            disabled={currentStep === 0 && !quizStarted}
            className={`px-5 py-2.5 rounded-2xl text-xs font-extrabold transition-all flex items-center gap-1.5 cursor-pointer ${
          ...
          {!quizStarted ? (
            <div className="flex gap-2">
              {currentStep === totalLessons - 1 && (
                <button
                  onClick={onComplete}
                  className="px-4 py-2.5 rounded-2xl bg-[#b45309] hover:bg-[#92400e] text-white font-extrabold text-xs shadow-md shadow-[#b45309]/20 transition-all active:scale-95 flex items-center gap-1 cursor-pointer"
                >
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-2xl bg-[#b91c1c] hover:bg-[#991b1b] text-white font-extrabold text-xs shadow-md shadow-[#b91c1c]/20 transition-all active:scale-95 flex items-center gap-1.5 cursor-pointer"
              >
                <span>{currentStep === totalLessons - 1 ? '随堂考辨' : '下一条'}</span>
